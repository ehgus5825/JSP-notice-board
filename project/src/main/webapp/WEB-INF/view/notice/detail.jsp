	...
	<body>	
		...
		<div id="body">
			<div class="content-container clearfix">	
				<!-- --------------------------- aside --------------------------------------- -->
				<aside class="aside">
					<!-- 접속중인 세션이 관리자라면 ADMIN PAGE  -->
					<c:if test="${sessionScope.userMember.role == 'ADMIN'}">
						<h1>ADMIN PAGE</h1>
					</c:if>
					<!-- 접속중인 세션이 관리자가 아니라면 Community  -->
					<c:if test="${sessionScope.userMember.role != 'ADMIN'}">
						<h1>Community</h1>
					</c:if>
					<nav class="menu text-menu first margin-top">
						<h1>메뉴</h1>
						<ul>
							<li><a href="/noticeList">뻘글</a></li>							
						</ul>
					</nav>						
				</aside>
				<!-- --------------------------- main --------------------------------------- -->
				<main>
					<h2 class="main title">Community</h2>
					...
					<div class="margin-top first">
						<h3 class="hidden">Community 내용</h3>
						<table class="table">
							<tbody>
								<!-- list.jsp에서 함께 보낸 id에 해당하는 Notice 행(n)을 받아옴 -->
								<tr>
									<th>제목</th>
									<!-- n의 제목 -->
								...
								<tr>
									...
									<td class="text-align-left text-indent" colspan="3">
										<!-- 등록일을 화면에 표시할 때 사용할 패턴 : yy/MM/dd hh:mm:ss / jstl의 태그로 날짜 포맷을 지정 -->
								...
								<tr>
									...
									<td>${n.writerId}</td>
									<th>조회수</th>
									<!-- n의 조회수 / 패턴 타입을 number로 지정 / jstl의 태그로 숫자 포맷을 지정 -->
								...
								<tr>
									...
									<!-- n의 첨부파일 -->
									<!-- forTokens는 jstl의 태그이며 특정 구분자를 통해서 items을 var로 분리하여 반복문을 돌림 -->
									<c:forTokens var="fileName" items="${n.files}" delims="," varStatus="st">
										<!-- style이라는 변수를 만들어서 zip이라고 끝나는 파일에는 특정 스타일을 적용시킴 -->
										<c:set var="style" value="" />
										<c:if test="${fn:endsWith(fileName, '.zip')}">
											<c:set var="style" value="font-weight: bold; color:red;" />
										</c:if>
										<!-- 첨부파일을 다운할 수 있고 열 수 있도록 a태그로 생성(링크를 통해 다운) / jstl의 기능인 fn을 통해서 파일명을 모두 대문자로 -->
					...
					<div class="margin-top text-align-center">
						<!-- 태그 클릭시 memberMyPage로 돌아감. 가지고 왔던 페이지, 검색필드, 검색어를 가지고 돌아감-->
						<a class="btn btn-list" href="/noticeList?p=${param.p}&f=${param.f}&q=${param.q}">목록</a>
						<!-- 글 수정과 글삭제는 관리자의 권한이기 때문에 관리자에게만 보이도록 설정 -->
						<c:if test="${sessionScope.userMember.role == 'ADMIN'}">
							<!-- 태그 클릭시 memberMyUpdate에 현재 글의 id를 가지고 가서 글을 수정함 -->
							<a class="btn-text btn-default" href="/noticeUpdate?id=${n.nid}">수정</a>
							<!-- id를 설정해 놨기 때문에 클릭시 밑의 스크립트의 이벤트 활성화 -->
		                    <a class="btn-text btn-default" id="del">삭제</a>
		                    <script type="text/javascript">
					            ...
					            btn.addEventListener('click', function(){
					            	...
					            	if(confirm('글을 정말 삭제하시겠습니까?')){		
					            		// jstl의 out 태그를 통해서 el 태그를 자바스크립트의 값으로 변환
					            		var del = "<c:out value="${n.nid}" />";
					            		// 삭제 확인이 되면 삭제하고자 하는 id를 지니고 memberMyDelete로 이동
						            	location.href="/noticeDelete?id=" + del;
							        }
					            	// 삭제 취소
									else{
										return;
					...
					<div class="margin-top">
						<table class="table border-top-default">
							<tbody>
								<tr>
									<th>다음글</th>
									<!-- 다음글로 받아온 nextN이 비어있지 않다면 -->
									<c:if test="${nextN.nid != 0}">
										<td colspan="3"  class="text-align-left text-indent">
											<!-- 다음글로 이동할 수 있는 a 태그를 출력 / 태그의 값은 글의 제목 / 이동할 때 list에서 detail로 왔던 것과 동일하게 페이지, id, 검색필드, 검색어를 함께 보냄-->
											<a class="text-blue text-strong" href="/noticeDetail?p=${param.p}&id=${nextN.nid}&f=${param.f}&q=${param.q}">
												${nextN.title}
											</a>
										</td>
									</c:if>
								...
								<tr>
									<th>이전글</th>
									<!-- 이전글로 받아온 pervN이 비어있지 않다면 -->
									<c:if test="${pervN.nid != 0}">
										<td colspan="3"  class="text-align-left text-indent">
											<!-- 이전글로 이동할 수 있는 a 태그를 출력 / 태그의 값은 글의 제목 / 이동할 때 list에서 detail로 왔던 것과 동일하게 페이지, id, 검색필드, 검색어를 함께 보냄-->
											<a class="text-blue text-strong" href="/noticeDetail?p=${param.p}&id=${pervN.nid}&f=${param.f}&q=${param.q}">
												${pervN.title}
											</a>
										</td>
									</c:if>
