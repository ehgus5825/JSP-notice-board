	...
	<body>
		...
		<div id="body">
			<div class="content-container clearfix">
				...
				<!-- --------------------------- main --------------------------------------- -->
				<!-- 공지사항 목록이 표시될 부분 -->
				<!-- 공지사항은 관리자의 뷰와, 비회원과 회원의 뷰, 두가지로 다르게 표시되어야 한다. -->
				<main class="main">
					...
					<form method="post">
							...
							<table class="table">
								<thead>
									<tr>
										...
										<th class="w60">조회수</th>
										<!-- 사용자가 관리자일 경우 공개와 삭제 컬럼을 추가적으로 보여줌 -->
										<c:if test="${sessionScope.userMember.role == 'ADMIN'}">
											<th class="w40">공개</th>
											<th class="w40">삭제</th>
										</c:if>
								...
								<!-- 사용자가 관리자일 경우 보여줄 공지사항 목록 -->
								<c:if test="${sessionScope.userMember.role == 'ADMIN'}">
									<tbody>
										<!-- 목록 중 하나의 내용이 한 행에 표시되어야 하기 때문에 ListController로부터 받아온 list2의 내용을 출력 -->
	                            		<!-- list2를 기준으로 반복문을 돌려서 하나(n)를 빼냄-->
										<c:forEach var="n" items="${list2}">
											<tr>
												...
												<td class="title indent text-align-left">
													...
													<a href="noticeDetail?p=${page}&id=${n.nid}&f=${param.f}&q=${param.q}">
								...
								<!-- 사용자가 회원이나 비회원일 경우 보여줄 공지사항 목록 -->
								<c:if test="${sessionScope.userMember.role != 'ADMIN'}">
									<tbody>
										<!-- 목록 중 하나의 내용이 한 행에 표시되어야 하기 때문에 ListController로부터 받아온 list1의 내용을 출력 -->
	                            		<!-- list1를 기준으로 반복문을 돌려서 하나(n)를 빼냄-->
						...
						<!-- 관리자일 때와 아닐 때 마지막 페이지 번호가 다르기 때문에 noticeList에서 다른 값을 받아옴  -->
						<!-- 관리자가 아닐 때 -->
						<c:if test="${sessionScope.userMember.role != 'ADMIN'}">
							<!-- 전체 개수(count1)를 10으로 나누고 올림을 한 다음 정수로 바꾸기 위해 소수점을 삭제시킴 / 목록당 게시물이 10개이기 때문 -->
							<c:set var="lastNum" value="${fn:substringBefore(Math.ceil(count1/10), '.')}"/>
						</c:if>
						<!-- 관리자일 때 -->
						<c:if test="${sessionScope.userMember.role == 'ADMIN'}">
							<!-- 전체 개수(count2)를 10으로 나누고 올림을 한 다음 정수로 바꾸기 위해 소수점을 삭제시킴 / 목록당 게시물이 10개이기 때문 -->
							<c:set var="lastNum" value="${fn:substringBefore(Math.ceil(count2/10), '.')}"/>
						</c:if>
						...
						<div class="text-align-right margin-top">
							...
							<!-- 회원이랑 관리자라면 사용할 수 있는 글쓰기 기능 / 비회원이라면 보이지 않는다 -->
							<c:if test="${sessionScope.userMember != null}">
								<a class="btn-text btn-default" href="/noticeReg">글쓰기</a>
							</c:if>
						</div>
					</form>
					<!-- 이전, 페이저, 다음 -->
