		...
		window.onload = function() {
		    ...
		    document.getElementById('btn').onclick = function() {
		    	...
		    	if(confirm('글을 정말 수정하시겠습니까?')){	
			    	...
			    	// 비어있지 않다면 submit => action="/noticeUpdate"
					frm.submit();
			    	return;
		        }
		    	// 등록 취소
	    ...
	    <div id="body">
	        <div class="content-container clearfix">
	            ...
	            <main>
					...
					<!-- 글 수정의 예외처리가 될 form -->
					<!-- 파일을 보내야하기 때문에 인코딩 방식이 multipart/form-data -->
	                <form id="frm" method="post" enctype="multipart/form-data">
	                    <div class="margin-top first">
	                        <h3 class="hidden">Community 입력</h3>
	                        <table class="table">
	                            <tbody>
	                            	<!-- 기존이 내용이 표시되어야하기 떄문에 UpdateController로부터 받아온 n의 내용을 출력 -->
	                            	<!-- (n은 해당 게시물(id)의 글 내용임) -->
	                                <tr>
	                                    <th>제목</th>
	                                    <td class="text-align-left text-indent text-strong text-orange" colspan="3">
	                                    	<!-- n의 기존 제목을 출력 -->	
	                                        <input required type="text" name="title" value="${n.title}"/>
	                                    </td>
	                                </tr>
	                                <tr>
	                                    <th>첨부파일</th>
	                                    <!-- n의 기존 파일을 출력 -->
	                                    <td colspan="3" class="text-align-left text-indent"><input type="file" name="file" value="${n.files}"/> </td>
	                                </tr>
	                                <tr class="content">
	                                	<!-- n의 기존 내용을 출력 -->
	                                    <td colspan="4"><textarea required class="content" name="content">${n.content}</textarea></td>
	                                </tr>
	                                <tr>
	                                    <td colspan="4" class="text-align-right">
	                                    	<!-- n의 기존 공개여부를 출력 / 1이라면 체크 O 아니라면 체크 X -->
	                                    	<input class="vertical-align" type="checkbox" id="open" name="open" value="true" ${(n.pub == 1)?"checked":""}>
	                    ...
	                    <div class="margin-top text-align-center">
	                    	<!-- 수정 버튼 클릭시 위의 예외처리가 실행 -->
	                        <input class="btn-text btn-default" id="btn" type="button" value="수정" />
	                        <!-- 취소시 뒤로가기 -->
